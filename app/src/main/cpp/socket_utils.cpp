...
template <typename T> inline T read_exact_or(int fd, T fail) {
  T res;
  return sizeof(T) == xread(fd, &res, sizeof(T)) ? res : fail;
}

template <typename T> inline bool write_exact(int fd, T val) {
...
bool write_string(int fd, std::string_view str) {
  return write_usize(fd, str.size()) &&
         str.size() == xwrite(fd, str.data(), str.size());
}
} // namespace socket_utils
